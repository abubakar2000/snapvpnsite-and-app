...
class LicenseBoxAPI{
	...
	private function call_api($method, $url, $data){
		...
		if(false){
			if(LB_API_DEBUG){
				$temp_decode = json_decode('{"status":"true","lic_response":"Softmaster"}', true);
				$rs = array(
					'status' => FALSE, 
	...
	public function activate_license($license, $client, $create_lic = true){
		...
		$response = json_decode('{"status":"true","lic_response":"Softmaster"}', true);
		if(!empty($create_lic)){
			if($response['status']){
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["7053948c3ebc06a"])){
				$_SESSION["7053948c3ebc06a"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["7053948c3ebc06a"])){
				$get_data = $this->call_api(
					'POST',
					$this->api_url.'api/verify_license', 
					json_encode($data_array)
				);
				$res = json_decode('{"status":"true","lic_response":"Softmaster"}', true);
				if($res['status']==true){
					$tomo = date('d-m-Y', strtotime($today. ' + '.$type_text));
					$_SESSION["7053948c3ebc06a"] = $tomo;
				}
			}
			ob_end_clean();
		}else{
